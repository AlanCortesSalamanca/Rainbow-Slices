...
begin
  for r in
    ...
    where conrelid = 'products'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) ilike any (array['%pieces_per_batch%', '%presentation%'])
  loop
...
alter table products
  add constraint products_presentation_pieces_per_batch_check
  check (
    (presentation = 'slice' and pieces_per_batch > 0)
    or (presentation in ('whole', 'mini', 'custom') and pieces_per_batch = 1)
  );

alter table production_batches
  add column if not exists production_mode text not null default 'recipe';

alter table production_batches
  drop constraint if exists production_batches_production_mode_check;

alter table production_batches
  add constraint production_batches_production_mode_check
  check (production_mode in ('recipe', 'manual'));

alter table production_batches
  alter column estimated_total_cost set default 0;

create index if not exists idx_production_batches_mode on production_batches(production_mode);
create index if not exists idx_finished_inventory_related_order_item_id on finished_inventory_movements(related_order_item_id);
create index if not exists idx_finished_inventory_related_order_id on finished_inventory_movements(related_order_id);
create index if not exists idx_finished_inventory_movement_type on finished_inventory_movements(movement_type);
create index if not exists idx_products_presentation on products(presentation);
...
begin
  ...
  select exists (
    select 1 from product_recipes where product_id = p_product_id
  ) into v_has_recipe;
  ...
  if v_has_recipe then
    ...
    for r in
      select
        ...
        pr.quantity_required * p_batches_quantity as required_quantity,
        coalesce(vis.current_stock, 0) as current_stock
    ...
    select coalesce(batch_cost, 0)
    into v_batch_cost
    from vw_product_recipe_costs
  ...
  insert into finished_inventory_movements (
    product_id,
    production_batch_id,
    movement_type,
    quantity,
  ...
  values (
    ...
    v_units_produced,
    case
      when v_production_mode = 'manual' then 'Producción manual sin receta'
      else 'Producción registrada con receta'
    end
  );
...
begin
  ...
  set
    status = 'cancelled',
    payment_status = case
      when status in ('in_preparation', 'ready') or requires_production then 'cancelled_no_refund'::payment_status
